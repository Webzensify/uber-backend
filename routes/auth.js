...
router.post("/register", async (req, res) => {
  const { role, name, mobileNumber, email, otp, aadhaarNumber, gender } =
    req.body;
...
router.get("/get", authenticateUser, async (req, res) => {
  try {
    const user = req.user; // Retrieved from the `authenticateUser` middleware
    if (!user) {
    ...
    return res
      .status(200)
      .json({ msg: "User details retrieved successfully", entity: user });
  } catch (err) {
...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    return res
      .status(200)
      .json({ msg: `${role} logged in successfully`, entity, token });
  } catch (err) {
    return res
      .status(500)
...
router.post("/send-otp", async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ msg: "Error sending OTP", error: err.message });
  }
});
...
router.post("/addDriver", authenticateUser, async (req, res) => {
  ...
  try {
    ...
    if (entity)
      return res.status(400).json({ msg: `${role} already registered` });

    if (!licenseNumber || !aadhaarNumber) {
      return res.status(400).json({
        msg: "License number and aadhaar number are required for driver",
      });
    }
